...
public class TRES extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        limelight.pipelineSwitch(0); // 确认 Limelight 后台 Pipeline 索引
        limelight.start();
        ...
        while (opModeIsActive()) {
            ...
            boolean targetValid = (result != null && result.isValid());
            ...
            if (gamepad2.a && targetValid) {
                double tx = result.getTx();
                if (Math.abs(tx) > deadzone) {
                    // 公式：新位置 = 旧位置 - (偏差 * 灵敏度)
                    // 如果两个舵机转反了，就把 -= 改成 +=
                    currentPos += tx * sensitivity;
                }
